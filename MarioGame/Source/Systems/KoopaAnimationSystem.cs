...
public class KoopaAnimationSystem : BaseSystem, IRenderableSystem
{
    ...
    public void Draw(GameTime gameTime, IEnumerable<Entity> entities)
    {
        if (entities != null)
        {
            var enemies = entities.WithComponents(typeof(AnimationComponent), typeof(ColliderComponent), typeof(MovementComponent), typeof(KoopaComponent));
            foreach (var entity in enemies)
            {
                ...
                //.CurrentFrame++;
                //f (animation.CurrentFrame >= animation.Textures.Count)
                {
                    if (!enemy.IsAlive)
                    {
